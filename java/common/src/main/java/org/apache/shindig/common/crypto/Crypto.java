...
public final class Crypto {
  ...
  //
  // 指定された長さのランダムなバイト列をつくり、その各々のバイトを16進変換した結果を
  // 連結した文字列を返す。
  //
  // たとえば、3 を指定すると、"1a026c" 等を返すことになる。1バイトは2文字になるので、倍の長さの文字列が返される。
  //
  /**
  ...
  // 指定された長さのランダムな数字列を返す。
  //
  // たとえば、5 を指定すれば、"80246" とか "18426" のような文字列を返す。
  //
  /**
  ...
  //
  // HMAC Sha1 にもとづいたメッセージ認証符号を返す。in から key を使って MAC（メッセージ認証符号）を生成する。
  //
  /**
   * HMAC sha1
   * 
   ...
   * 
   * in を key で処理したメッセージ認証符号と expected が合致しているかチェックする。
   * 合致していなければ例外をスローする。
   * 
   * @param key
   * @param in
   ...
   * of the cipher text.
   * 
   * AES アルゴリズム / CBC モード / PKCS パディング方式で暗号化する。暗号化に使用した初期化ベクタと
   * 暗号文を連結したバイト列を返す。初期化ベクタはランダムバイトを生成してそれを利用する。
   * 
   * @param key
   * @param plain
   * 
   ...
   * 
   * 暗号化キー key, 初期化ベクタ iv を使って、平文 plain を暗号化した暗号文を返す。
   *
   * @param key
   * @param iv
   * @param plain
