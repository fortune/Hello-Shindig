...
//
// プロパティファイルから読みだした Key=value のそれぞれについて、
// @Named(key) がついた String に value をバインドさせるための Guice モジュール。
//
...
public class PropertiesModule extends AbstractModule {
  ...
  protected void configure() {
    //
    // プロパティファイルで定義された、各 Key=value について、
    // Module 内の configure メソッドで
    //
    // bind(String.class).annotatedWith(Names.named(Key)).toInstance(value);
    //
    // とやるのと同じ効果がある。これにより、@Named(key) を指定している String への Inject が必要なとき、
    // value の値がセットされる。
    //
