...
public class DefaultHandlerRegistry implements HandlerRegistry {
  ...
  //
  // serviceMethodPathMap は REST 用のハンドラを登録するためのマップ。
  // REST サービス名から検索し、それが返すマップに対し、HTTP メソッド名から検索すると、RestPath オブジェクトの Set が得られる。
  // RestPath オブジェクトには、RestHandler とそれに対応するパスが入っている。
  // Map service - > method -> { RestPath, ...}
  //
  // たとえば、People 系のサービスだと people というサービス名だろうから、それをキーに検索する。
  // その結果に対し GET で検索すると、RestPath の Set が返ってくる。そこで、ユーザからのリクエストパスに
  // 適合する RestPath を探し出し、それがもつ RestHandler を使ってサービスを実行する。適合する RestPath が
  // 複数あるような場合、最初に見つかったのを使うようだ。なので、そういう事態は避けねばならない。
  //
  ...
  // RPC 用のハンドラを登録するマップ。
  //
  // サービス名（たとえば people）と RPC メソッド名とを "." でつないだ文字列をキーとし、RPC ハンドラを値とする。
  // 
  ...
  //
  // BaseRequestItem から別の型（JSON を使って変換する）ときに必要になる。
  //
  ...
  public DefaultHandlerRegistry(Injector injector,
    ...
    //
    // このオブジェクトは、Injector 内で生成され、パラメータが Inject されるだろうから、
    // その場合、injector はこのオブジェクトを生成した Injecotr 自身になるだろう。
    //
  ...
  public void addHandlers(Set<Object> handlers) {
    for (final Object handler : handlers) {
      //
      // 各ハンドラオブジェクトのタイプ（Class）とそれを得るための Provider を取得する。
      //
      Class<?> handlerType;
      ...
      //
      // ハンドラは、Service アノテーションが付いていなければならない。
      // この Service アノテーションによって、そのハンドラを起動するための Path を指定するから。
      //
      ...
      //
      // ハンドラ内の Public かつ Operation アノテーションが付いているメソッドが
      // 登録の対象になる。
      // Operation アノテーションに指定されている httpMethods[] がそのメソッドに対応する HTTP メソッドになる。
      //
      // つまり、Service アノテーションに指定されている Path と Operation アノテーションに指定されている
      // HTTP メソッド名によって、Rest および Rpc を実行するためのメソッドが特定できる。
      //
   ...
   * 
   * path は、サーバに送られてきたリクエストパスである。これは、サービス名とパスから構成されると見なされる。
   * たとえば、
   * 
   * 	/people/boo/foo
   * 
   * ならば、people がサービス名、boo と foo が残りのパスだ。
   * 
   * method が HTTP メソッドである。サービス名と method 、そして残りのパスから適切な RestHandler が
   * 返される。
   */
  ...
  //
  // REST サービスを実行するための RestHandler を作成し、登録する。
  //
  // handlerProvider は、RestHandler となるメソッドを定義しているクラスオブジェクトを生成するための Provider。
  //
  // service は、RestHandler となるメソッドを定義しているクラスに指定されている Service アノテーション。これがサービスの名称、パスを定義する。
  // たとえば、Person 系の REST サービスなら、"/people/{userId}+/{groupId}/{personId}+" という文字列が
  // HTTP リクエストの PathInfo になっているだろうが、people がサービス名称、"/{userId}+/{groupId}/{personId}+" が
  // パスになる。
  //
  // op は RestHandler となるメソッドにつけられた Operation アノテーション。
  // そのメソッドが起動されるときの HTTP メソッド名たち（REST の場合）、メソッドの名前（RPC 時に使用されるときの名前。REST では関係ないと思われる。）、
  // 上の Service で定義したパスを上書きするためのパス、メソッドで起動されるサービスで使用される body パラメータのパラメータ名等を指定する。
  //
  // m が RestHandler となるメソッドだ。
  // 
  private void createRestHandler(Provider<?> handlerProvider,
    ...
    try {
      ...
      //
      // RPC ではメソッド名を指定して実行する。普通はメソッド名をそのまま使うが、Operation アノテーションで
      // 変更することができる。REST では無用の操作だが、ExecutionListener 用にやっている。
      //
  ...
  private static class MethodCaller {
    ...
    //
    // method は、Rest および Rpc サービス用に実行されるメソッド。
    // isRest は、これが REST 用なら true, RPC 用なら false になる。
    //
    // mechod は引数をとるなら、１つだけであり、それは RequestItem 型か、RequestItem 型から変換可能な型でないといけない。
    //
    // RequestItem 型は、ハンドラのメソッドを実行するときに使われる共通のパラメータ型のようなものだ。RequestItem 型でない
    // 型を引数にとる method はおそらく RPC から起動されるものだろう。おそらく RPC の引数となる JSON オブジェクトから
    // BaseRequestItem オブジェクトが作成され、そこから目的の型へ変換されるので、BaseRequestItem から変換可能な型でないと
    // いけない。
    //
    // この規約に合致していないと、このコンストラクタで例外がスローされるか、将来、このオブジェクトをとおしてそのメソッドを
    // 起動したときに例外がスローされる。
    //
    /**
    ...
    public MethodCaller(Method method, boolean isRest) throws NoSuchMethodException {
      ...
      // method の引数の型が RequestItem 型だった場合、それは BaseRequestItem 型だと見なされる。
      // RequestItem はインターフェースなので、そのようにしないと、将来、この method を実行するときに
      // 引数となるオブジェクトを生成できないからだ。したがって、その型は、BaseRequestItem か、そのサブ型でないといけない。
      //
      ...
      //
      // method の引数を生成するときに使う型を決める。引数の型が RequestItem 型（かそれのサブ型）でない場合は
      // BaseRequestItem 型を使う。その引数の型へと変換するときに使う中継として BaseRequestItem を使うからだ。
      //
      Class<?> requestItemType = inputIsRequestItem ? inputClass : BaseRequestItem.class;
    
      //
      // REST 用か RPC 用かで method を起動するときに使用する RequestItem 型のコンストラクタが
      // 異なる。RPC の場合は、JSON から作成できるようにしている。
      //
      // ということは、RequestItem を実装した独自の型を使う場合、そのクラスは、
      // ここでのコンストラクタを用意しないといけないということだ。
      //
    ...
    // method を実行する。
    //
    // handler は method を定義してあるクラスのインスタンスでなくてはならない。
    // item はクライアントからの HTTP リクエストを元に getRequestItem メソッドを使って
    // 作成されたものだ。
    //
  ...
  //
  // REST サービスのパス（/サービス名/.../.../...）とそのためのハンドラをもつクラス
  //
  /**
  ...
  static class RestPath implements Comparable<RestPath> {
    ...
    //
    // パス部分が定数値かシングル変数かマルチ変数かを表す定数。
    // パス部分は、{userid}+/@all/{personid} などのようになっているが、
    // {} でくくられているのが変数である。それに + がついていると、, で区切られたマルチ変数になる。
    //
    enum PartType {
    ...
    //
    // たとえば、{userid}+/{groupId/{personId}/foo ならば、
    //
    // partName, type がそれぞれ、
    //
    //	userId, PLURAL_PARAM
    //	groupId, SINGULAR_PARAM
    //	personId, PLURAL_PARAM
    //	foo, CONST
    //
    // になる。
    //
    ...
    public RestPath(String path, RestInvocationHandler handler) {
      ...
      constCount = tmpConstCount;	// this.operationPath にはサービス名も含まれてるんだからこれは当然 > 0 になるだろ。
      lastConstIndex = tmpConstIndex;
      this.handler = handler;
    }

    //
    // requestPathParts がこの RestPath に合致するかをチェックする。
    // 合致するならば、Rest ハンドラを返す。この Rest ハンドラはもともとのハンドラを
    // ラップしたものだ。何故ラップしてるかというと、パス中の変数の値をもてるようにするためだ。
    // たとえば、この RestPath のパスが /people/{userid}/@all/{personid}+ だったとしよう。
    // requestedPathParts[] が { "people", "kazu", "@all", "jane,roy" } だとすると合致するが、
    // userid という変数に kazu という値、personid という変数に jane と roy を対応付けたものをラップの中に
    // もつようにする。そうすることで、このラップに対して execute したとき、ユーザのリクエストパラメータの値とそのラップ中の
    // 変数をマージしたものを Rest ハンドラに渡して起動できるようになる。
    //
    // requestePathParts[] の先頭はサービス名である。
    //
    /**
