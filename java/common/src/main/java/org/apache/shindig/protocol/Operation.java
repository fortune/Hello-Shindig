 ...
 * 
 * このアノテーションがつけられたサービスハンドラ（Service アノテーションが付けられているクラス）のメソッドが
 * REST および RPC の処理をする。
 */
...
public @interface Operation {
   ...
   * 
   * 処理を実行するときの HTTP メソッド名を指定する。REST で使われる。
   */
  String[] httpMethods();
   ...
   * 
   * アノテーションを付けられたメソッド名が、そのまま RPC のメソッド名となるが、
   * それを上書きできる。メソッド名が hoge でも、このフィールドで echo という名前を
   * 指定すると、RPC メソッド名として echo を使える。
   */
  String name() default "";
}
