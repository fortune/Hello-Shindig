...
//
// Parser 関連の DI を定義するモジュール。
//
// Gadget XML をパースする GadgetHtmlParser の実装と、そこで使われる DOMImplementation の実装を
// 指定している。DOMImplementation インターフェースの提供するメソッドを使って、XML 文書をパースできるのであった。
//
/**
...
public class ParseModule extends AbstractModule {
  ...
  //
  // DOM Level 3 で提供された DOMImplementation インターフェースの実装の取得方法を Provider 内で
  // 使っている。
  //
  /**
  ...
  public static class DOMImplementationProvider implements Provider<DOMImplementation> {
    ...
    public DOMImplementationProvider() {
      try {
        //
        // DOmImplementationRegistry をとおして DOMImplementation の実装を取得できる。
        // システムプロパティ org.w3c.dom.DOMImplementationSourceList の値から実装を特定するようだ。
        //
