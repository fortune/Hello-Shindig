...
public class HttpRequestHandler {
  ...
  private HttpApiResponse execute(String method, HttpApiRequest httpApiRequest,
    ...
    try {
      //
      // リクエストに使う HttpRequest オブジェクトを構築していく。
      //
      ...
      if (gadgetUri == null) {
        //
        // JSON-RPC は Gadget XML アプリケーションからリクエストされたということが
        // 前提になっている。そうであれば、その Gadget XML アプリケーションの URL が
        // セキュリティトークンに含まれているはずだから。
        //
        throw new ProtocolException(HttpServletResponse.SC_BAD_REQUEST,
      ...
      //
      // 元のリクエストについてきた authz パラメータの値をチェックする。
      // "signed" か "oauth" か指定なしであり、それによって、外部へのリクエストの方法が変わってくるわけだ。
      //
      ...
      if (req.getAuthType() != AuthType.NONE) {
        //
        // 外部サーバへ認証リクエストを出す場合（2-legged oauth or 3-legged oauth）、
        // そのためのパラメータを作成するために、セキュリティトークン、その他が必要になってくるので
        // セットしておく。
        //
      ...
      //
      // ここまでで作成した HttpRequest オブジェクトを使って
      // 外部サーバへリクエストを実行し（キャッシュを使うかもしれない）、結果を手に入れる。
      //
  ...
  protected Uri getGadgetUri(SecurityToken token, HttpApiRequest httpApiRequest) {
    //
    // Gadget XML アプリケーションからのリクエストから作成されたセキュリティトークンには、
    // その Gadget XML アプリケーションの URL が入っているはずである。
    //
