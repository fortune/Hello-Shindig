...
public class MakeRequestHandler {
  ...
  public void fetch(HttpServletRequest request, HttpServletResponse response)
    ...
    //
    // 元のリクエストから、外部サーバへ送信するための HttpRequest オブジェクトを作成する。
    //
    HttpRequest rcr = buildHttpRequest(request);

    // Serialize the response
    //
    // 外部サーバへのリクエストを実行し、レスポンスを取得する。キャッシュを使っているかもしれない。
    //
  ...
  protected HttpRequest buildHttpRequest(HttpServletRequest request) throws GadgetException {
    //
    // request から、転送先 URL を取得し、それが適正かどうかを検証する。
    //
    ...
    //
    // 外部サーバへの HttpRequest を作成する。
    //
    // リクエストから container パラメータの値もセットしているが、
    // これは使用しないようである。signed リクエストを指示されているなら、container の値が
    // 2-legged oauth での oauth_consumer_key の値になるはずだが、これはセキュリティトークンから
    // 取得するはずだから。
    //
    ...
    //
    // 転送リクエストのリクエストヘッダをセットする。
    //
    // 元リクエストの headers パラメータで指定される。ここに
    //
    // {header 名を URL エンコードしたもの}={header 値を URL エンコードしたもの}&...
    //
    // 全体を URL エンコードしたものが指定されている。
    //
    ...
    //
    // 転送リクエストの HTTP メソッドが POST であり、それの Content-type ヘッダが指定されていない場合、
    // デフォルトの Content-type を設定する。
    //
    ...
    //
    // 元リクエストの nocache パラメータで、キャッシュを使用するかどうかが指定される。1 ならキャッシュを使用しないし、
    // 転送リクエストで得られたレスポンスをキャッシュすることもしない。
    // 
    req.setIgnoreCache("1".equals(request.getParameter(Param.NO_CACHE.getKey())));

    //
    // 元リクエストの gadget パラメータには、リクエスト元の Gadget XML アプリケーションの URL が
    // 入っている。
    //
