...
public class Processor {
  ...
  public Gadget process(GadgetContext context) throws ProcessingException {
    ...
    //
    // Gadget XML を格納しているサーバがブラックリストに入っていたらエラーを返す。
    //
    ...
    try {
      //
      // GadgetSpec を取得すると、（キャッシュされていないなら）Gadget XML をもっているサーバにアクセスして
      // 取得する。
      //
      // GadgetSpec 中には、取得した Gadget XML のすべてが入っている。
      // ModulePref にはどんなことが定義されているか？タイトルは何か？ どういう Content があるか？ それらの
      // view は何か？等々。
      //
      GadgetSpec spec = gadgetSpecFactory.getGadgetSpec(context);
      
      //
      // Gadget XML 中の __MSG_変数名__ などを変数の値で置き換える。
      //
      spec = substituter.substitute(context, spec);

      //
      // Gadget オブジェクトにすべてまとめ上げる。
      // このオブジェクトを使って、HTML へと変換される。
      //
      return new Gadget()
          .setContext(context)
  ...
  //
  // context で要求されている View と Gadget が用意している View を
  // 比べて、"current" な View を決定する。
  //
  /**
  ...
  private View getView(GadgetContext context, GadgetSpec spec) {
    ...
    if (view == null) {
      //
      // 合致する View が見つからない場合、
      // Container で定義されている View の別名を調べてみる。
      //
