...
public class JsonRpcHandler {
  ...
  public JSONObject process(JSONObject request) throws RpcException, JSONException {
    ...
    // クライアントから、
    //
    // {"context":{"country":"default","language":"default","view":"canvas","container":"default"},"gadgets":[{"url":"http://kazu.fortunefield.com/opensocial/examples/data_request.xml","moduleId":1}]}
    //
    // のような JSON が metadata 要求のリクエストとして送られてきている。gadgets に metadata 要求対象の Gadget XML アプリケーションの配列、
    // context でそれのコンテキストを指定している。
    //
    ...
    //
    // クライアントの Gadget XML レンダリング要求（/gadgets/ifr=... というリクエストによる）では
    // GadgetContext の実装として HttpGadgetContext を使用し、HttpServletRequest から Gadget XML レンダリングのために
    // GadgetContext オブジェクトを生成した。
    //
    // ここでは metadata 要求を処理するために GadgetContext の実装として JsonRpcGadgetContext を使用し、
    // JSON から GadgetContext を生成する。
    //
    ...
    //
    // Gadget の数だけそれぞれ個別に処理させる。
    //
    // Gadget XML の URL へアクセスし（キャッシュが使用されるかも）、
    // その Gadget XML アプリケーションの XML ファイルを取得し、
    // そこから Gadget XML 仕様を読み取るのだ。
    //
    ...
    //
    // それぞれ個別に処理した結果を１つの JSON にまとめる。
    //
    // {"gadgets": [ {....}, {....} ] }
    //
    // の形で、１つ１つの要素がそれぞれの Gadget の metadata が入っている形式の JSON 配列になる。
    //
  ...
  //
  // java.util.concurrent の並行処理メカニズムに対して
  // metadata 取得・生成の Job をさせるためのクラス。
  //
  protected class Job implements Callable<JSONObject> {
    ...
    public JSONObject call() throws RpcException {
      try {
        //
        // Gadget XML を取得し、それの仕様を取得し、
        // それらから metadata を表現する JSON を生成する。
        //
