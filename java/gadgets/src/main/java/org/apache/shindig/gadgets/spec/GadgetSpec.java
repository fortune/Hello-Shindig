...
//
// Gadget XML の仕様を格納するオブジェクト。
//
// Gadget XML で定義されている内容、すなわち
//
//	・タイトル
//	・作者
//	・Require 要素の feature 属性で要求されている Feature。"opensocial-0.8" 等。OpenSocial 仕様にないような
//	 適当な値を指定してもよいが、結果としてエラーになるだろう。
//	・Content や、そこで指定されている view（home や profile 等々）など。
//
// を格納する。よって、キャッシュされていないなら、このオブジェクトを作る時点で、Gadget XML サーバへと
// リクエストする。
//
