...
//
// Feature 定義のレジストリとなるクラス。
//
// Feature というのは、core や opensocial-0.8 などなど。Gadget XML 内の ModulePrefs 要素内で
//
//	<Require feature="opensocial-0.8" />
//
// のようにして、その Gadget で必要となる Feature を指定する。core は必ず使うので指定しなくても指定したことになる。
//
// Shindig 側では、自分がサポートする Feature をこのレジストリに登録しておく。
// そのために、Feature を定義した XML ファイルを読み出す。これら XML ファイルは、次のような
// 形式で記述されている。
//
//	<feature>
//		<name>core.io</name>
...
//	</feature>
//
// name 要素で Feature 名を定義し、dependency 要素で、この Feature が依存している他の Feature を
// 指定する。それらの依存先の Feature もこれと同様の形式の XML で定義されている。
// 後は、この Feature をサポートするための JavaScript ファイルだろう。Gadget XML を HTML に変換した後に
// これら JavaScript ファイルが script タグに指定されることになる。
// 
/**
...
//
// features パラメータで、Feature を定義する XML ファイル名を記述した
// ファイルのリストを指定する。
//
/**
 *
