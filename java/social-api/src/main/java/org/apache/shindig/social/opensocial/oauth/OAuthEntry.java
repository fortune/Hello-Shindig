 ...
 * Authorization.
 * 
 * OAuth のトークン情報および認証情報を格納する。トークンのタイプは何か？
 * そのトークンの Consumer は誰か？（Consumer Key）、そのトークン（当然アクセストークン）を認証したユーザ（リソースオーナー）は誰か？
 * 等々。
 * 
 * 2-legged ではなく 3-legged OAuth である。
 */
public class OAuthEntry implements Serializable {
  ...
  //
  // トークンのタイプ。
  // リクエストトークン or アクセストークン or 使用不可トークン
  //
