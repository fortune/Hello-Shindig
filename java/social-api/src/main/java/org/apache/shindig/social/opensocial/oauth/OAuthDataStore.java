...
//
// Consumer key から Consumer 情報（当然、Consumer secret が含まれる）を検索できるようにしたり、
// リクエストトークンおよびアクセストークンを格納し、それに結びついたトークン情報を検索できるようにしたり、
// それらトークン情報を更新したりできるようにするためのインターフェース。
//
// データベースと結びついたクラスで実装することになるだろう。
//
...
public interface OAuthDataStore {
   ...
   * 
   * 2-legged oauth リクエスト用のセキュリティトークンを作成する。consumerKey は、アプリケーションの ID である。
   * userId は、2-legged oauth リクエスト中の xoauth_requestor_id パラメータで指定されたであろう ownerId である。
   * メソッドでは、userId が本当に consumerKey で指定されるアプリケーションをインストールしているかどうかを確認しなければならない。
   *
