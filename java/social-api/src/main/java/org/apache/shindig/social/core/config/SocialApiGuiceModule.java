...
//
// Social API 関連コンポーネントの DI をしている。JavaScript API の opensocial.* や Restful API で
// 使われる機能だろう。
//
...
public class SocialApiGuiceModule extends AbstractModule {
  ...
  protected void configure() {
    ...
    //
    // REST API でのレスポンスを適切な形式（Json, XML, Atom）に変換するコンバータを Bind
    //
    ...
    //
    // HTTP リクエストからセキュリティトークンを取得するためのハンドラを DI する。
    //
    bind(new TypeLiteral<List<AuthenticationHandler>>(){}).toProvider(
        AuthenticationHandlerProvider.class);

    //
    // DefaultGuiceModule でも同じところにハンドラを Bind していたから、これらのハンドラと合わさることになる。
    // だが、ここではハンドラの Class 情報を Bind しており、DefaultGuiceModule ではハンドラオブジェクトを
    // Bind している。この違いは何だろう？ org.apache.shindig.protocol.DefaultHandlerRegistry で
    // うまく処理している。
    //
  ...
  //
  // 使用するハンドラを提供するためのフック。これらのハンドラを変更したり追加したりしたい場合は、
  // サブクラス化して、このメソッドをオーバライドしてやればよい。
  //
  /**
